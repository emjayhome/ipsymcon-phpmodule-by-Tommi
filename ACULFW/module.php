 ...
 * @file
 *
 * ACULFW IPSymcon PHP Splitter Module  for heliflieger a-culfw receiver
 *
 * @author Manfred Jantscher modified from Thomas Dressler
 * @copyright Thomas Dressler 2011-2017, Manfred Jantscher 2017
 * @version 1.0
 * @date 2017-12-08
 */
...
/** @class CUL
 *
 * A-CULFW IPSymcon PHP Splitter Module Class
 * for heliflieger A-CULFW receiver
 * tested with IPS 4.3 A-CULFW 1.26.01 a-culfw Build: 271 (2017-09-18_20-23-44) nanoCUL433 (F-Band: 433MHz)
 * and Brennenstuhl RCS 1000 N System
 *
...
class CUL extends T2DModule
{
     ...
     * @param string $line Received record from A-CULFW
     * @return void
     */
    public function Parse(string $line)
    {
        ...
        elseif (!preg_match("/^\s*$/", $line)) {
            ...
            elseif (preg_match("/^(V\s*[0-9\.]+)\s*(?:a-culfw).*/", $line, $res)) {
                $vers = $res[1];
                $versid = $this->GetIDForIdent('Version');
